...
int main(){
    ...
    while (operacion != "EOF"){ //se verifica que el archivo aun no termina
        ...
        if (operacion == coeficiente){
            ...
            lista_Polinomios[num_polinomio].moveToPos(g_a);
            tElemLista g = lista_Polinomios[num_polinomio].getValue(); //busca el polinomio que se necesita 
            float g_b = g.coeficiente; //busca el exponente dentro del polinomio
            resultado.write((char*)&g_b, sizeof(float)); //se escribe la solucion en el archivo de respuesta
        };

    };
    resultado.close();  //cierra los .txt que se abrieron 
    polinomios.close();
    delete lista_Polinomios;  //borra la lista_Polinomios
    return 0;
};